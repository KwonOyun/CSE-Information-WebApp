...
public class information3{
	...
	public static void doMain() throws Exception{
		...
		t.scheduleAtFixedRate(new Crawling3(), 0, 60000);   //60초마다 주기적인 크롤링 실행
...
class Crawling3 extends TimerTask{    //크롤링을 실행하는 클래스
	int i = 0;
	static Document doc;
	information3 info = new information3();
	public void run(){
		try{
			FileReader prefr = new FileReader("C:\\Users\\권오윤\\workspace\\WebTermJSP\\WebContent\\information3.json");  //원래 있던 제이슨 파일을 읽어온다.
			BufferedReader prebr = new BufferedReader(prefr);  //원래 있던 제이슨 파일을 읽어온다.
			String filename = "C:\\Users\\권오윤\\workspace\\WebTermJSP\\WebContent\\information3.json"; //새로 생성할 제이슨 파일 이름 설정
			File file = new File(filename);  //파일 객체 생성
			FileOutputStream fos = new FileOutputStream(filename);  

			doc = Jsoup.connect("http://computer.cnu.ac.kr/index.php?mid=saccord").get();  //Jsoup라이브러리의 connect함수를 이용해 크롤링한 내용을 doc에 저장
			...
			FileReader afterfr = new FileReader("C:\\Users\\권오윤\\workspace\\WebTermJSP\\WebContent\\information3.json");  //새로 완성된 제이슨 파일을 읽어온다.
	...
	public void mailsend(){  //smtp메일을 발송하는 함수
		...
		String content = "사업단소식이 업데이트 되었습니다!! 확인하세요!!";         //메일 내용
		String from = "oyoon33@gmail.com";  //보내는 사람 메일
		String subject = "충남대학교 컴퓨터공학과 사업단소식 업데이트!!";  //메일 제목
